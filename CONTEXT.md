...
**Sustainable fork CI**:
A validation model that preserves relevant cross-platform coverage and conceptual alignment with
upstream while remaining independently operable with infrastructure available to the fork.
_Avoid_: Infrastructure parity, mirrored CI, Linux-only CI
...
Platform-native tests whose success is required by the merge gate for every essential platform.
...
**Extended validation**:
The full post-merge validation suite across extended platforms and specialized build paths,
