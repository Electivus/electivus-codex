...
-> Result<()> {
    ...
    let repository_id = create_fake_rollout(
        ...
        Some(CoreGitInfo {
            ...
            branch: None,
            repository_url: Some(
                SanitizedGitUrl::try_from("git@example.com:acme/project.git")
    ...
    let unrelated_id = create_fake_rollout(
        ...
        Some(CoreGitInfo {
            ...
            branch: None,
            repository_url: Some(
                SanitizedGitUrl::try_from("https://example.com/acme/other.git")
                    .expect("repository URL should be valid"),
            ),
        }),
    )?;
...
async fn thread_list_project_cwd_ignores_configured_origin_outside_repository() -> Result<()> {
    ...
    let configured_origin_id = create_fake_rollout(
        ...
        Some(CoreGitInfo {
            ...
            branch: None,
            repository_url: Some(
                SanitizedGitUrl::try_from("https://example.com/acme/configured.git")
                    .expect("repository URL should be valid"),
            ),
        }),
    )?;
...
async fn thread_list_project_cwd_oversized_origin_falls_back_to_exact_cwd() -> Result<()> {
    ...
    let same_oversized_identity_id = create_fake_rollout(
        ...
        Some(CoreGitInfo {
            ...
            branch: None,
            repository_url: Some(
                SanitizedGitUrl::try_from("https://example.com/acme/project.git")
                    .expect("repository URL should be valid"),
            ),
        }),
    )?;
