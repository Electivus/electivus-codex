...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let sqlite_home_from_env = resolve_sqlite_home_env(&resolved_cwd);
        let sqlite_home_was_explicit = cfg.sqlite_home.is_some() || sqlite_home_from_env.is_some();
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or(sqlite_home_from_env)
