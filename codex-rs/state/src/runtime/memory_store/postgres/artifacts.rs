...
impl PostgresMemoryStore {
    pub(crate) async fn publish_memory_generation(
        ...
        let artifact_count = i32::try_from(artifacts.len())?;
        let total_bytes = artifacts.iter().try_fold(0_i64, |total, artifact| {
        ...
        let published_at: i64 = sqlx::query_scalar(AssertSqlSafe(format!(
    ...
    pub(crate) async fn load_active_memory_generation(
        ...
        Ok(Some(MemoryGeneration {
            generation_id: first.try_get("generation_id")?,
            completed_watermark: first.try_get("completed_watermark")?,
            published_at: first.try_get("published_at")?,
            artifacts,
        }))
    }
}
