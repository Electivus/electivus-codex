...
fn schema_versions_outside_the_compatibility_range_are_actionable() {
    ...
    assert_eq!(
        ensure_compatible_schema_version(schema, 10)
            .expect_err("newer schema should be rejected")
            .to_string(),
        "PostgreSQL schema `isolated_namespace` is at version 10, newer than the maximum supported version 9; upgrade Codex before using this namespace"
    );
}
