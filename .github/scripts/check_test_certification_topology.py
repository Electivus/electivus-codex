...
def validate_topology(
    ...
    checks = (
        ...
        ("retained evidence", "if: always()" in verify and 'verify_test_certification.py" verify' in verify and '--expected-sha "${CANDIDATE_SHA}"' in verify and "GITHUB_STEP_SUMMARY" in verify and "actions/runs/${GITHUB_RUN_ID}" in verify and "if: always()" in upload and "actions/upload-artifact@" in upload and "name: test-certification-${{ matrix.test_id }}-${{ env.CANDIDATE_SHA }}" in upload and "path: ${{ env.EVIDENCE_DIR }}" in upload and "if-no-files-found: error" in upload and "retention-days: 90" in upload),
