...
def validate_topology(
    ...
    checks = (
        ...
        ("retry-free exact nextest", "--cargo-profile ci-test" in sequence and "--cargo-profile ci-test" in verifier and "--retries 0" in sequence and "--run-ignored=only" in sequence and "-E \"test(=${{ matrix.identity }})\"" in sequence and "--allow-retries" not in sequence),
        ("single JUnit testcase", 'junit_source="${GITHUB_WORKSPACE}/codex-rs/target/nextest/default/junit.xml"' in sequence and "CARGO_TARGET_DIR" not in sequence and "check_nextest_junit.py" in sequence and "--expected-testcases 1" in sequence),
        ...
        ("ordinary reactivation path", "target: x86_64-unknown-linux-gnu" in x64 and "uses: ./.github/workflows/rust-ci-full-nextest-platform.yml" in x64 and "--run-ignored" not in shard and "check_nextest_junit.py" in shard and "--allow-retries" in shard and "RETRY_ELEMENTS" in junit),
