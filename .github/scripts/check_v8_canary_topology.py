...
def validate_topology(
    ...
    checks = (
        ...
        ("metadata fail safe", "canary_reason: ${{ steps.changes.outputs.canary_reason }}" in metadata and "canary_required: ${{ steps.changes.outputs.canary_required }}" in metadata and "canary_required=true" in detect and "detector_status=0" in detect and "detector exited with status" in detect and "${#detector_lines[@]} -eq 2" in detect and "^canary_required=(true|false)$" in detect and "^canary_reason=([[:print:]]{1,240})$" in detect and 'canary_required="${detector_lines[0]#canary_required=}"' in detect and "classifier returned malformed output" in detect and "canary_reason=${canary_reason}" in detect and "GITHUB_STEP_SUMMARY" in detect),
