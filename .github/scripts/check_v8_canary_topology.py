...
def validate_topology(
    ...
    windows_source = _job(canary, "build-windows-source")
    ...
    checks = (
        ...
        ("metadata fail safe", "canary_reason: ${{ steps.changes.outputs.canary_reason }}" in metadata and "canary_required: ${{ steps.changes.outputs.canary_required || 'true' }}" in metadata and "windows_source_required: ${{ steps.changes.outputs.windows_source_required || 'true' }}" in metadata and "canary_required=true" in detect and "windows_source_required=true" in detect and "detector_status=0" in detect and "detector exited with status" in detect and "${#detector_lines[@]} -eq 3" in detect and "^canary_required=(true|false)$" in detect and "^canary_reason=([[:print:]]{1,240})$" in detect and "^windows_source_required=(true|false)$" in detect and 'canary_required="${detector_lines[0]#canary_required=}"' in detect and 'windows_source_required="${detector_lines[2]#windows_source_required=}"' in detect and "classifier returned malformed output" in detect and "canary_reason=${canary_reason}" in detect and "windows_source_required=${windows_source_required}" in detect and "GITHUB_STEP_SUMMARY" in detect),
        ...
        ("conditional Windows source build", "needs: metadata" in windows_source and "if: ${{ needs.metadata.outputs.windows_source_required == 'true' }}" in windows_source and windows_source.count("runner: windows-2025") == 2 and "target: x86_64-pc-windows-msvc" in windows_source and "target: aarch64-pc-windows-msvc" in windows_source and "V8_FROM_SOURCE: \"1\"" in windows_source and "--features v8_enable_sandbox" in windows_source and "continue-on-error:" not in windows_source),
