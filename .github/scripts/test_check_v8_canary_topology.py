...
class V8CanaryTopologyTests(unittest.TestCase):
    ...
    def test_v8_canary_mutations_fail_closed(self) -> None:
        ...
        cases = (
            ...
            ("metadata fail safe", 0, canary.replace("windows_source_required=true", "windows_source_required=false", 1)),
            ...
            ("conditional Windows source build", 0, canary.replace("needs.metadata.outputs.windows_source_required == 'true'", "needs.metadata.outputs.windows_source_required != 'false'")),
            ("conditional Windows source build", 0, canary.replace("target: aarch64-pc-windows-msvc", "target: x86_64-pc-windows-msvc")),
