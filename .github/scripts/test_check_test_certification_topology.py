...
class TestCertificationTopologyTests(unittest.TestCase):
    ...
    def test_certification_contract_mutations_fail_closed(self) -> None:
        ...
        cases = (
            ...
            ("retry-free exact nextest", 0, workflow.replace("--retries 0", "--retries 1")), ("retry-free exact nextest", 0, workflow.replace("--cargo-profile ci-test", "--cargo-profile test")),
            ("retry-free exact nextest", 0, workflow.replace("--run-ignored=only", "--run-ignored=all")), ("retry-free exact nextest", 1, verifier.replace("--cargo-profile ci-test", "--cargo-profile test")),
