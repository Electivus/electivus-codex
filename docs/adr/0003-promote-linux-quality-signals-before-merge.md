...
shard sharing the x64 nextest archive, and Change-triggered validation for the Linux V8 matrix. A
test retry leaves the gate red; inherited flaky ignores must be certified or fixed, and a
...
automation retains only Extended validation that has not been promoted. Docker remote-executor,
macOS, Windows, and new coverage or sanitizer tooling remain deferred until separately certified.
