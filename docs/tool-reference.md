...
| `exec` | `features.code_mode`/`code_mode_only`, or model tool mode. | `freeform` | {runtime value}<br><br>Configured yield default is {runtime value} ms; effective range is {runtime value}-{runtime value} ms. Explicit requests outside the range are clamped. | `codex-rs/core/src/tools/code_mode/execute_spec.rs` |
